...
module.exports = function (options) {
	...
	return function (astTree, fileData) {
		if (!fileData.data.node.fields[keyName]) return
		const now = moment()
		const publishDate = moment(new Date(fileData.data.node.fields.date))
		if (publishDate.isBefore(now)) return
		// Fetch our encryption key
		const key = fileData.data.node.fields[keyName]
